...
export function parseAlert({ from = '', subject = '', text = '', html = '' } = {}) {
  if (/notifications@github\.com/i.test(from)) return parseGithub({ subject, text, html })
...
export function resolveRepo(parsed, { clients = [], map = {} } = {}) {
...
export function loadClientMap(osDir) {
  try {
    ...
    for (const d of dirs) {
      ...
      out.push({ name: fm.name || d.name, github: fm.github || null, repo: fm.repo || null, site: fm.site || null })
    }
    return out
  } catch {
    return []
