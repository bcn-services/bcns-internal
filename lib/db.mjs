...
// notify's backlog read: rows at `stage` with no `notified` marker yet. The
// dedupe has to happen in SQL, before the limit — filtering after it let a
// stage with 50+ already-notified rows hide every new one (2026-09-10: 15
// call_due rows never mailed). The key expression mirrors notifyKey(). A
// "no answer" call waits until next_touch_at, not the next hourly tick.
export function unnotifiedByStage(sql, stage, { limit = 50 } = {}) {
  const q = read(sql)
  return q`
    select s.* from selectable_businesses s
    where s.stage = ${stage}
      and (s.stage <> 'call_due' or s.next_touch_at is null or s.next_touch_at <= now())
      and not exists (
        select 1 from events e
        where e.job = 'notify' and e.kind = 'notified'
          and e.detail->>'key' = s.id::text || ':' || s.stage ||
            case when s.stage = 'call_due'
              then ':' || coalesce(to_char(s.next_touch_at at time zone 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'), 'none')
              else '' end
      )
...
// `research` is stored as jsonb holding a double-encoded JSON string (the
// column itself is a JSON string, not an object), so `#>>'{}'` unwraps the
// text value and `::jsonb` parses it before `review_count`/`rating` can be
// read out of it. Candidates are found through the view (the read guard);
// the promotion UPDATE below writes `businesses` directly by id, same
// convention as `updateBusiness`.
export function promoteNoEmail(sql, { limit = 20 } = {}) {
  const q = read(sql)
  return q`
    with candidates as (
      select id from selectable_businesses
      where stage = 'no_email'
      order by
        ((research #>> '{}')::jsonb ->> 'review_count')::numeric desc nulls last,
        ((research #>> '{}')::jsonb ->> 'rating')::numeric desc nulls last
      limit ${limit}
    )
    -- next_touch_at = now() gives notify a fresh call_due key: a row that was
    -- notified before it was parked would otherwise read as already told.
    update businesses set stage = 'call_due', next_touch_at = now(), updated_at = now()
    where id in (select id from candidates)
    returning *
  `
}

export function promotedToday(sql) {
  return sql`
    select count(*)::int as count from events
    where job = 'qualify' and kind = 'promoted' and created_at::date = current_date
  `
}
