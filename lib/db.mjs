...
// How many times this mailbox message has already failed. `events` is the
// poller's only marker store, so the dead-letter counter reads it rather than
// adding state; bounded by the handful of rows one bad uid can produce.
export function messageFailureCount(sql, uid) {
  return sql`
    select count(*)::int as count from events
    where job = 'poll' and kind in ('error', 'dead_letter') and detail->>'uid' = ${String(uid)}
  `
}
