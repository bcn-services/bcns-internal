...
export async function runSkill({
  ...
  try {
    ...
    // A non-zero exit puts claude's reason in the stdout envelope, not stderr.
    let said = ''
    try {
      said = JSON.parse(String(err.stdout ?? ''))?.result ?? ''
    } catch {
      said = String(err.stdout ?? '')
    }
    const tail = said ? `; said: ${String(said).slice(0, 400)}` : ''
    throw new Error(`skill run failed for ${JSON.stringify(command)}: ${err.message}${tail}`)
  }
