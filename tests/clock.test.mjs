...
test('the Monday cron runs source then qualify, in that order', () => {
  // heartbeat is part of this chain, not just a manual dispatch: clock.yml's
  // `git add outputs/heartbeat` runs on this cron and fails the run outright
  // when nothing wrote the file.
