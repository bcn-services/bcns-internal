...
- 04:32Z 09-05: after Nate said "take care of it": PR #8 merged 04:27Z (59fc116) via GitHub UI as nseluga; default branch → main (API PATCH). Test repo delete stopped at GitHub sudo/passkey wall (tab left open on settings/delete) — Nate. Scheduler crons/ref/PAT: not a new script, rerun `scripts/scheduler-setup.sh` (idempotent, ref main, production crons, takes the new PAT) — Nate, gcloud is classifier-blocked here. bcns-os f05e7f6: four memory files (runner token, Cloud Scheduler dispatch, Gmail alias reply quirks, persist credentials methodically).
- 09-06: hand-offs closed by Nate. Test repo deleted (API 404) after passkey sudo. Scheduler: gcloud auth had flipped to the HMC account (wizard login opened in the default School profile → PERMISSION_DENIED on bcns-leads); fixed with `gcloud config set account nseluga@bcn-services.com` + `--no-launch-browser` login pasted into Work profile. Crons/ref set by four `jobs update http` without --update-headers (existing PAT kept; no rotation needed, it was never exposed). Read-back: all four ENABLED, production crons, ref main, Authorization header present. zsh gotcha: unquoted $VAR does not word-split, inline the flags.
- 2026-09-06 template resync: Nate ruled the skill wording ("we've built a mock-up") is canonical; lib/template.mjs re-copied, the skill's "never claim a demo exists" rule removed at his call. 5 stored drafts patched in place via SQL; DiRocco + Sureflow drafts predate the compliment template entirely and must be reset to `qualified` (research minus draft) so personalize rebuilds them — classifier blocked that UPDATE, handed to Nate. `*` in SEND_ALLOWED_RECIPIENTS now opens the prospect list (touch.mjs assertAllowed); ignored for NOTIFY.
